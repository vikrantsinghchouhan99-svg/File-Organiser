...
1. The script looks inside a folder named **test_files**.  
2. It reads each file’s extension (like `.pdf`, `.jpg`, `.mp4`, `.txt`, etc.).  
3. It creates folders (if they don’t exist already).  
4. Then moves files into the correct folders according to type.  
5. It also uses a log file to record what changes were made or if any error occurred.
...
- Creating and updating log files using `logging` module.  
...
1. Keep your sample files inside a folder named `test_files`.  
2. Place that folder in the same directory as `main.py`.  
